...
import { Activity, Box, Layers, ShieldCheck, SquareStack } from 'lucide-react';
...
export function RevitStatusBar() {
  ...
  const healthScore = useBIMStore((s) => s.healthScore);
  const issueCount = useBIMStore((s) => s.validationIssues.length);

  const fpsColor = fps >= 50 ? 'text-emerald-400' : fps >= 30 ? 'text-amber-400' : 'text-rose-400';
  const healthColor = healthScore >= 90 ? 'text-emerald-400' : healthScore >= 60 ? 'text-amber-400' : 'text-rose-400';
  ...
  return (
    <div className="h-7 shrink-0 bg-slate-900 border-t border-slate-800 flex items-center px-3 gap-4 text-[11px] text-slate-500">
      ...
      <span className={`flex items-center gap-1 ${healthColor}`}>
        <ShieldCheck size={12} />
        {healthScore}%{issueCount > 0 && <span className="text-slate-500">({issueCount} issue{issueCount === 1 ? '' : 's'})</span>}
      </span>
